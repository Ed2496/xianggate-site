...
# 程式版本戳記（改程式時手動 +1；報告頭部與 console 會顯示，用來一眼判斷頁面是否為最新程式產出）
SCRIPT_VERSION = "1.6 (累加折線+no-cache)"
...
<div class="wrap">
  ...
  <div class="sub">{html.escape(summary['week_key'])} ｜ 週起 {html.escape(summary['week_start'])} ｜ <b style="color:#0F766E">產出 {html.escape(generated)}</b> ｜ v{html.escape(SCRIPT_VERSION)} ｜ 匯入日期基準：<b>{'建立日期' if summary.get('date_basis')=='created' else '修改日期'}</b>{' ｜ <b>公開安全模式</b>' if PUBLIC_SAFE else ''}</div>
...
def main():
    ...
    ap.add_argument("--mainfile", default="index.html",
                    help="主輸出檔名（預設 index.html；run_all.bat 傳 weekly.html 避免覆蓋儀表板首頁）")
    args = ap.parse_args()
    ...
    # 覆寫主輸出檔（預設 index.html；run_all.bat 用 --mainfile weekly.html）
    index_path = os.path.join(out_dir, args.mainfile)
    ...
    print(f"[相閘週報] {summary['week_key']}  as-of {summary['asof']}  v{SCRIPT_VERSION}")
